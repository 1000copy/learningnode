...
可以通过log子命令来查看repo内已经有的版本。这里显示目前已经有一个版本（commit）。版本标识为 d2fe108e92df8d827f6ec237db85693dbd6a1eab，作者是1000copy <1000copy@gmail.com>等等。值得注意的是，版本标识每次是不同的，即使你按照我的命令一模一样的执行，这个标识符也是不同的。但是在不同的电脑不同的时间，都可以使用git log内列出的标识符来标识提交。
