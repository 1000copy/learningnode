...
有了暂存区，我们可以把当前的工作区修改分为几组放入暂存区然后提交。但是之前的分批提交仅仅演示到文件层面。而Git还提供了比文件更细的颗粒度来做提交。就是说，哪怕是在一个文件内的多个修改也是可以拆分开来，然后如文件一样放入暂存区的。它的最小的颗粒度可以达到一行。Git称这样在文件内继续细分的修改为hunk（块）。
