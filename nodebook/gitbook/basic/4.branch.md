...
环境构建完成。现在执行以下命令，以便把当前分支rebase到master上：
...
这个输出消息中，我们特别留意到：rI的父修订本来是r1，而现在变成了r2。git-rebase会把指定的分支的全部修订搬移到当前分支上，完成操作后，两个分支的历史合并为一条单线。
...
我们看到仓库修订回到r2。使用命令`$cat file1`，从而可以发现file1此修订下的内容是妥当的：
