...
HTTP协议下的Git托管涉及到了Web Server的配置、CGI知识、特定的OS以及版本的配置方法差别,是相对最复杂的，因为篇幅所限，本节仅仅列出特定操作系统、特定Web服务器下的配置。我会使用OS X作为配置案例。其他配置的读者可以参考我给出的配置过程。我的具体实验环境如下：
...
的命令，以http协议方法访问服务器上的repo.git仓库。留意URL的协议部分是http。

我们在目录/var/www/git内作为仓库存放的基础目录，在此目录内创建一个名为repo.git的仓库：
